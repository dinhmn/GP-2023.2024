    ...
    <template v-slot:body>
      ...
      <div class="grid w-full grid-cols-4 gap-2">
        <div class="col-span-1 form-search h-[100%] flex flex-col gap-4 rounded-md py-2">
          <h3 class="text-[#17b1ea] font-bold text-xl block text-center py-2 bg-slate-700">
            Danh mục sản phẩm
          </h3>
          <div>
            <ul class="flex flex-col gap-2 text-sm bg-slate-700">
              <li
                v-for="item in count"
                :key="item"
          ...
          <div class="grid grid-cols-4 gap-3">
            ...
            >
              <router-link :to="{ name: 'DetailProduct', params: { id: item } }">
                <Item
                  :price="Number(item * 16456 + 80000)"
                  :priceSale="Number(item * 92340 + 70000)"
